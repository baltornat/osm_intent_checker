                        ...
                        for key in total_resources.keys():
                            if "cpu" in key:
                                if key.endswith("_requests") and "cpu" in requests:
                                    total_resources[key] += convert(requests["cpu"]) * int(replicas)
                                elif key.endswith("_limits") and "cpu" in limits:
                                    total_resources[key] += convert(limits["cpu"]) * int(replicas)
                            elif "memory" in key:
                                if key.endswith("_requests") and "memory" in requests:
                                    total_resources[key] += convert(requests["memory"]) * int(replicas)
                                elif key.endswith("_limits") and "memory" in limits:
                                    total_resources[key] += convert(limits["memory"]) * int(replicas)
                        ...
                        for volume in volumes:
                            ...
                            for key in total_resources.keys():
                                if key == "sizeLimit" and key in sizes:
                                    total_resources[key] += convert(sizes[key]) * int(replicas)
                        ...
                        for volume_claim in volume_claim_template:
                            ...
                            for key in total_resources.keys():
                                if key == "storage" and key in requests:
                                    total_resources[key] += convert(requests[key]) * int(replicas)
